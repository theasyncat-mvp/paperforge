...
export default function MergeTool() {
  ...
  const handleMerge = async () => {
    ...
    try {
      ...
      setStatus("done");
      setMessage(`Merged ${files.length} files → ${outputFile}`);
    } catch (err) {
      setStatus("error");
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-xl font-semibold mb-1">Merge PDFs</h2>
        <p className="text-sm text-zinc-500 dark:text-zinc-400">
          Select multiple PDF files and combine them into one.
        </p>
      </div>
      ...
      <button
        onClick={handleMerge}
        disabled={files.length < 2 || status === "running"}
